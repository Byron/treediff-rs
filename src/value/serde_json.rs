...
impl Value for SerdeJson {
    ...
    fn items<'a>(&'a self) -> Option<Box<Iterator<Item = (Self::Key, &'a Self::Item)> + 'a>> {
        match *self {
            ...
            SerdeJson::Bool(_) |
            SerdeJson::Null => None,
            SerdeJson::Array(ref inner) => {
                Some(Box::new(inner.iter().enumerate().map(|(i, v)| (Key::Index(i), v))))
            }
            SerdeJson::Object(ref inner) => {
                Some(Box::new(inner.iter().map(|(s, v)| (Key::String(s.clone()), v))))
            }
        }
    }
}
    ...
    fn set(&mut self, keys: &[Self::Key], v: &Self::Item) {
        if keys.len() == 0 {
            *self = v.clone();
            ...
            fn runup_array_or_value<'a>(array: &'a mut Vec<SerdeJson>,
                                        target_index: usize,
                                        key_index: usize,
